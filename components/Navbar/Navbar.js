...
import { addCorporationJsonLd, addFaqJsonLd, addTourJsonLd, dynamicMetaData } from "utils/utils";
...
const NavBar = (props) => {
    ...
    useEffect(() => {
        ...
        const setSubmenuObjects = (slugs) => {
            const items = slugs.filter(el => el.attributes.display_name).map(el => {
                const { attributes: { slug, display_name } } = el
                return { name: display_name, route: `/tours/${slug}`, isSubMenu: true }
            });
    ...
    const subMenuItems = navRoutes.filter(item => item.isSubMenu).map(item => (
        {
            key: item.name,
            label: (<Link href={item.route} style={{ fontSize: 16, fontFamily: 'Poppins', color: 'black' }}>{item.name}</Link>)
        }
    ));
                    ...
                    `}>
                        <div className="container">
                            <span className="navbar-brand text-uppercase font-weight-bold">
                                <Link href="/">
                                    {navbarClass
                                        ? <LazyLoadImage id="cpt-logo" alt="logo" style={{ width: 90, height: '100%' }} src='/images/logo-green.webp' />
                                        : <LazyLoadImage id="cpt-logo-white" alt="logo" style={{ width: 125, height: '100%' }} src='/images/logo-white-full.webp' />
                                    }
                                </Link>
                            </span>
                            ...
                            <div id="navbarSupportedContent" className={`collapse navbar-collapse justify-content-center`}>
                                <ul className="navbar-nav align-items-center text-nowrap">
                                    ...
                                    <li className="dropdown nav-item pl-4">
                                        <DropdownTours subMenuItems={subMenuItems}>
                                            <span
                                    ...
                                    <li className="nav-item btn-cpt">
                                        <span className="nav-link text-light">
                                            <Link scroll={false} className="nav-contact-btn" href="/#contact">
                                                Contact
                                            </Link>
                                        </span>
                                    </li>
