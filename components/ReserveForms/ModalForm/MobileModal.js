...
import { calculateTotalPrice, CustomInput, extractNumber, findMinNumber } from "utils/utils";
...
const MobileModal = (props) => {
    ...
    return (
        <>
            <button className={`btn-reserve ${isHidden ? 'm-checkout' : ''}`}
                onClick={() => {
                    setMainModal(true);
                    setIsHidden(true);
                    resetTourSelection();
                }}>
                {!isNotFixedPrice ? 'Reserve' : 'Request Now'}
            </button>
            <Modal
                title={null}
                open={isMainModal}
                onOk={() => setMainModal(false)}
                onCancel={() => { setMainModal(false); setIsHidden(false) }}
                footer={null}
                destroyOnClose={true}
            >
